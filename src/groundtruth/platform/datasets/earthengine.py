...
:class:`~groundtruth.contracts.errors.DataUnavailableError` until the reducer chain
...
from groundtruth.contracts.errors import ConfigurationError, DataUnavailableError
from groundtruth.contracts.identifiers import DatasetRef
from groundtruth.contracts.ports import ObservationSpec
...
class EarthEngineProvider:
    ...
    def provenance(self, spec: ObservationSpec) -> Provenance:
        ...
        return Provenance.computed(
            ...
            method=f"annual-median-composite/{spec.indicator.value}",
            stage="ingestion",
            parameters={
                "collection": COLLECTIONS[self.collection],
                "scale_m": self.scale_m,
                "max_cloud_fraction": spec.max_cloud_fraction,
    ...
    def dataset(self, indicator: Indicator) -> DatasetRef:
        """Return the pinned dataset reference for an indicator."""
        return DatasetRef(
            dataset_id=self.collection.replace("_", "-"),
            version="unpinned",
            provider="google-earth-engine",
            asset=COLLECTIONS[self.collection],
        )
...
class EarthEngineCovariateProvider:
    ...
    def covariates(self, unit: UnitRef) -> dict[str, float]:
